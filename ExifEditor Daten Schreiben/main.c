...
/* dimensions of image */
static const unsigned int image_jpg_x = 58;
...
int main()
{
	...
	entry = init_tag(exif, EXIF_IFD_0, EXIF_TAG_DATE_TIME_ORIGINAL);
	exif_set_long(entry->data, FILE_BYTE_ORDER, 'b');

	entry = init_tag(exif, EXIF_IFD_0, EXIF_TAG_MAKE);
	exif_set_long(entry->data, FILE_BYTE_ORDER, '0');

	entry = init_tag(exif, EXIF_IFD_EXIF, EXIF_TAG_MODEL);
	exif_set_long(entry->data, FILE_BYTE_ORDER, 'a');
	...
	memcpy(entry->data + 8, Herrsteller, sizeof(Herrsteller) - 1);
